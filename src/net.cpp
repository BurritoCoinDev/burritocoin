...
void CNode::copyStats(CNodeStats &stats, const std::vector<bool> &m_asmap)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (BurritoCoin users should be well used to small numbers with many decimal places by now :)
...
bool CConnman::OutboundTargetReached(bool historicalBlockServingLimit)
{
    ...
    if (historicalBlockServingLimit)
    {
        ...
        uint64_t buffer = timeLeftInCycle / Params().GetConsensus().nPowTargetSpacing * MAX_BLOCK_SERIALIZED_SIZE_WITH_MWEB;
