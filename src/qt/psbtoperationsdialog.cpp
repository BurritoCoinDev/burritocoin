...
void PSBTOperationsDialog::saveTransaction() {
    ...
    for (const CTxOut& out : m_transaction_data.tx->vout) {
        ...
        QString amount = BurritoCoinUnits::format(m_wallet_model->getOptionsModel()->getDisplayUnit(), out.nValue);
...
std::string PSBTOperationsDialog::renderTransaction(const PartiallySignedTransaction &psbtx)
{
    ...
    for (const CTxOut& out : psbtx.tx->vout) {
        ...
        tx_description.append(tr(" * Sends %1 to %2")
            .arg(BurritoCoinUnits::formatWithUnit(BurritoCoinUnits::BRTO, out.nValue))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(BurritoCoinUnits::formatWithUnit(BurritoCoinUnits::BRTO, *analysis.fee));
        ...
        tx_description.append(QString("<b>%1</b>: <b>%2</b>").arg(tr("Total Amount"))
            .arg(BurritoCoinUnits::formatHtmlWithUnit(m_client_model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void PSBTOperationsDialog::showStatus(const QString &msg, StatusLevel level) {
    ...
    switch (level) {
        case StatusLevel::INFO: {
            m_ui->statusBar->setStyleSheet("QLabel { background-color: #1e8e3e; color: #ffffff; border-radius: 4px; padding: 4px 8px; }");
            break;
        }
        case StatusLevel::WARN: {
            m_ui->statusBar->setStyleSheet("QLabel { background-color: #c47d0e; color: #1a0f05; border-radius: 4px; padding: 4px 8px; }");
            break;
        }
        case StatusLevel::ERR: {
            m_ui->statusBar->setStyleSheet("QLabel { background-color: #c0392b; color: #ffffff; border-radius: 4px; padding: 4px 8px; }");
            break;
        }
    }
    m_ui->statusBar->show();
