...
bool Intro::showIfNeeded(bool& did_show_intro, bool& prune)
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || gArgs.GetBoolArg("-resetguisettings", false))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/burritocoin"));
        did_show_intro = true;
     ...
     * override -datadir in the burritocoin.conf file in the default data directory
     * (to be consistent with burritocoind behavior)
     */
    if(dataDir != GUIUtil::getDefaultDataDirectory()) {
...
void Intro::setStatus(int status, const QString &message, quint64 bytesAvailable)
{
    ...
    case FreespaceChecker::ST_ERROR:
        ...
        ui->errorMessage->setStyleSheet("QLabel { color: #e0604d }"); // brand::DangerLight
        break;
    }
...
void Intro::UpdateFreeSpaceLabel()
{
    ...
    if (m_bytes_available < m_required_space_gb * GB_BYTES) {
        ...
        ui->freeSpace->setStyleSheet("QLabel { color: #e0604d }"); // brand::DangerLight
    } else if (m_bytes_available / GB_BYTES - m_required_space_gb < 10) {
        freeString += " " + tr("(%n GB needed for full chain)", "", m_required_space_gb);
        ui->freeSpace->setStyleSheet("QLabel { color: #f5a623 }"); // brand::Gold (caution)
    } else {
...
void Intro::UpdatePruneLabels(bool prune_checked)
{
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the BurritoCoin block chain.").arg(PACKAGE_NAME) + " " +
